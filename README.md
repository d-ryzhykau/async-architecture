...
Но т.к. каждый сервис будет использовать свою базу данных,
проблемы с базой данных одного сервиса не повлияют на другой сервис.
...
должно быть оповещены другие сервисы. Той же проблеме подвержен и Auth.
...
запросы после ответа с кодами состояния 400-500,
повторять запросы после ответа с кодом 503 только после паузы, и т.д.

Также проблемы с подключением не позволят пользователям пройти логин, т.к.
...
Finance. Этого можно избежать ценой дополнительных запросов между сервисами.
Считаю, что дублирование данных и постепенная синхронизация позволят снизить
нагрузку на систему в сравнении с использованием запросов между сервисами.
...
Тогда не было бы необходимости в логике проверки timestamp для разных событий.
